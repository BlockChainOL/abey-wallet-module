...
class Base85CodecAscii extends Codec<Uint8List, String> {
  static const _alphabet = '!"#\$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstu';
  const Base85CodecAscii();

  @override
  Converter<Uint8List, String> get encoder => const Base85EncoderAscii(_alphabet);
...
class Base85CodecZ extends Codec<Uint8List, String> {
  static const String _alphabet = r"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ.-:+=^!/*?&<>()[]{}@%$#";
  const Base85CodecZ();
...
class Base85CodecIPv6 extends Codec<Uint8List, String> {
  static const _alphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!#\$%&()*+-;<=>?@^_`{|}~';
  const Base85CodecIPv6();

  @override
  Converter<Uint8List, String> get encoder => const Base85EncoderIPv6(_alphabet);

  @override
  Converter<String, Uint8List> get decoder => const Base85DecoderIPv6(decodeBase58IPv6);
}
...
String _processEncodeInput({
  ...
  for (int i = 0; i < length; i += 4) {
    if (zeroCompressionEnabled && input[i] == 0 && input[i + 1] == 0 && input[i + 2] == 0 && input[i + 3] == 0) {
      output[offset] = 122;
      offset += 1;
      padding += 4;
      continue;
    }
    int value = (input[i] << 24) + (input[i + 1] << 16) + (input[i + 2] << 8) + input[i + 3];
    for (int b = 4; b >= 0; b--) {
