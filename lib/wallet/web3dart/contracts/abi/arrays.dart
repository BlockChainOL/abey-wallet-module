...
class FixedBytes extends AbiType<Uint8List> {
  ...
  @override
  bool operator ==(other) {
    return identical(this, other) || (other is FixedBytes && other.length == length);
  }
}
...
class FixedLengthArray<T> extends BaseArrayType<T> {
  ...
  @override
  bool operator ==(other) {
    return identical(this, other) || (other is FixedLengthArray && other.length == length && other.type == type);
  }
}
...
class DynamicLengthArray<T> extends BaseArrayType<T> {
  ...
  DecodingResult<List<T>> decode(ByteBuffer buffer, int offset) {
    ...
    final arrayType = FixedLengthArray<T>(type: type, length: lengthResult.data.toInt());
    final dataResult = arrayType.decode(buffer, offset + lengthResult.bytesRead);
  ...
  bool operator ==(dynamic other) {
    return identical(this, other) || (other is DynamicLengthArray && other.type == type);
  }
}
