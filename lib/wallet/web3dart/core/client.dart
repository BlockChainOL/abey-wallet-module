...
import 'filters.dart';
import 'transaction_signer.dart' as transaction_signer;
...
class Web3Client {
  ...
  Web3Client.custom(RpcService rpc, {this.socketConnector}) : jsonRpc = rpc {
    _filters = FilterEngine(this);
  }
  ...
  Future<T> _makeRPCCall<T>(String function, [List<dynamic>? params]) async {
    try {
      final data = await jsonRpc.call(function, params);
  ...
  Future<List<FilterEvent>> getLogs(FilterOptions options) {
    final filter = EventFilter(options);
    return _makeRPCCall<List<dynamic>>(
      'eth_getLogs',
      [filter.createParamsObject(true)],
    ).then((logs) {
  ...
  Future<Uint8List> signTransaction(
    ...
    return transaction_signer.signTransaction(
  ...
  Future<List<eip1559.Fee>> getGasInEIP1559() async {
    return eip1559.getGasInEIP1559(jsonRpc.url);
  }
  ...
  Stream<String> addedBlocks() {
    return _filters.addFilter(NewBlockFilter());
  }
