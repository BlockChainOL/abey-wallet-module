...
class NewBlockFilter extends _Filter<String> {
  @override
...
class PendingTransactionsFilter extends _Filter<String> {
  @override
  bool get supportsPubSub => false;
...
class EventFilter extends _Filter<FilterEvent> {
  EventFilter(this.options);
  final FilterOptions options;

  @override
  _FilterCreationParams create() {
    return _FilterCreationParams('eth_newFilter', [createParamsObject(true)]);
  }
...
class FilterEngine {
  ...
  Future<void> _registerToPubSub(
    ...
    final peer = _client.connectWithPeer();
  ...
  Future uninstall(_InstantiatedFilter filter) async {
    ...
    if (filter.isPubSub && !_clearingBecauseSocketClosed) {
      final connection = _client.connectWithPeer();
